import { DoMoError } from "./types/errors.js";
 ...
 */
export async function authorizeRemoteOAuth(configuration, options = {}) {
    ...
    try {
        ...
        if (options.redirect)
            redirectSession = await options.redirect({ signal: controller.signal });
        const redirectUri = redirectSession?.redirectUri ?? options.redirectUri ?? resolved.redirectUri;
        if (!redirectUri) {
            throw new OAuthFlowError("Remote OAuth requires a loopback redirect session or an explicit redirectUri.", "redirect_required");
        }
        ...
        if (callback.state !== state)
            throw new OAuthFlowError("OAuth callback state did not match.", "state_mismatch");
        if (callback.error) {
            throw new OAuthFlowError(`The authorization server denied access${callback.errorDescription ? `: ${sanitizeOAuthText(callback.errorDescription)}` : "."}`, sanitizeOAuthText(callback.error));
        }
        if (!callback.code)
            throw new OAuthFlowError("OAuth callback did not contain an authorization code.", "missing_code");
        ...
        throwIfAborted(controller.signal);
        const response = await exchangeAuthorizationCode(resolved, client, callback.code, redirectUri, pkce.verifier, fetchFunction);
...
export async function resolveOAuthConfiguration(configuration, options = {}) {
    if (!configuration.serverUrl)
        throw new OAuthFlowError("OAuth serverUrl is required.", "configuration");
    ...
    if (configuration.authorizationEndpoint && configuration.tokenEndpoint)
...
export async function discoverOAuth(serverURL, options = {}) {
    const fetchFunction = options.fetch ?? defaultFetch;
    ...
    if (fromChallenge)
        addUniqueURL(resourceURLs, fromChallenge);
    const defaultResourceURL = wellKnown(server, "oauth-protected-resource", true);
    if (defaultResourceURL)
        addUniqueURL(resourceURLs, defaultResourceURL.toString());
    let resource;
    let issuers = [];
...
export async function refreshRemoteOAuth(configuration, credential, options = {}) {
    if (!credential.tokens.refreshToken)
        throw new OAuthFlowError("The OAuth credential has no refresh token.", "refresh_required");
    ...
    if (!client)
        throw new OAuthFlowError("Refreshing OAuth requires a client registration.", "client_required");
    ...
    if (resolved.resource ?? canonicalResource(resolved.serverUrl))
        fields.resource = resolved.resource ?? canonicalResource(resolved.serverUrl);
    if (resolved.scope)
        fields.scope = resolved.scope;
...
async function registerClient(configuration, redirectUri, fetchFunction, signal, clientName = "DoMoCode") {
    ...
    if (response.status < 200 || response.status >= 300)
        throw oauthHTTPError("Dynamic client registration failed", response.status, value);
    if (!isRecord(value) || typeof value.client_id !== "string")
        throw new OAuthFlowError("The dynamic registration response was invalid.", "client_registration_invalid");
    return {
...
function buildAuthorizationURL(configuration, clientId, redirectUri, challenge, state) {
    if (!configuration.authorizationEndpoint || !configuration.tokenEndpoint)
        throw new OAuthFlowError("OAuth endpoints are incomplete.", "configuration");
...
async function exchangeAuthorizationCode(configuration, client, code, redirectUri, verifier, fetchFunction) {
    if (!configuration.tokenEndpoint)
        throw new OAuthFlowError("OAuth token endpoint is missing.", "configuration");
    const fields = {
...
async function postTokenForm(endpoint, fields, fetchFunction) {
    if (!endpoint)
        throw new OAuthFlowError("OAuth token endpoint is missing.", "configuration");
    ...
    const value = await readJSON(response);
    if (response.status < 200 || response.status >= 300)
        throw oauthHTTPError("OAuth token exchange failed", response.status, value);
    if (!isRecord(value) || typeof value.access_token !== "string" || value.access_token.length === 0)
        throw new OAuthFlowError("The OAuth token response was invalid.", "token_invalid");
    if (value.token_type !== undefined && (typeof value.token_type !== "string" || value.token_type.toLowerCase() !== "bearer"))
        throw new OAuthFlowError("The authorization server issued a non-Bearer token.", "token_type");
    return {
...
function oauthHTTPError(prefix, status, value) {
...
async function fetchOptional(fetchFunction, url, signal) {
    try {
...
async function getJSON(fetchFunction, url, signal) {
...
function metadataCandidates(issuer) {
    const result = [];
    const add = (value) => { if (value && !result.some((candidate) => candidate.toString() === value.toString()))
        result.push(value); };
...
function wellKnown(url, suffix, pathInserted) {
...
function addUniqueURL(values, value) {
    try {
        const url = new URL(value);
        if (!values.some((item) => item.toString() === url.toString()))
            values.push(url);
    }
    catch {
...
function authParameter(name, header) {
...
function canonicalResource(serverURL) {
...
function normalizeCallback(value) {
    ...
    try {
        ...
        throw new OAuthFlowError("OAuth redirect was not a valid URL.", "callback_invalid");
    }
    const callback = {};
...
function sanitizeOAuthText(value) {
