...
export function renderMarkdownTranscript(messages, options = {}) {
    ...
    for (const message of messages) {
        ...
        if (message.role === "system")
            ...
            if (message.role === "assistant" && message.errorMessage)
                lines.push(`> Error: ${escapeMarkdown(message.errorMessage)}`, "");
        }
    }
    return trimTrailingBlankLines(lines).join("\n") + "\n";
}
export function renderHTMLTranscript(messages, options = {}) {
    const title = options.title ?? "DoMoCode transcript";
...
function markdownBlock(block, includeToolDetails) {
    switch (block.type) {
...
function htmlBlock(block, includeToolDetails) {
    switch (block.type) {
...
function trimTrailingBlankLines(lines) { while (lines.at(-1) === "")
    lines.pop(); return lines; }
function escapeMarkdown(value) { return value.replaceAll("\\", "\\\\").replaceAll("`", "\\`"); }
