...
import { metadata, nonNegativeCursor, object, optionalNumber, optionalString, requiredAlias, signalOptions, strings } from "./durableSupport.js";
...
export class AutomationClient {
    ...
    async list(options = {}) {
        ...
        const value = await this.transport.json(`/automations${query}`, { ...signalOptions(options.signal) });
        return requiredArray(value, "automations").map(decodeAutomationDefinition);
    }
    async register(definition, options = {}) {
        const value = await this.transport.json("/automation", {
            method: "POST",
    ...
    async get(id, options = {}) {
        const value = await this.transport.json(`/automation/${encodePathSegment(id)}`, { ...signalOptions(options.signal) });
        return decodeAutomationDefinition(value);
    }
    async enable(id, owner, options = {}) {
        return this.setEnabled(id, owner, true, options);
    }
    async disable(id, owner, options = {}) {
        return this.setEnabled(id, owner, false, options);
    }
    async setEnabled(id, owner, enabled, options = {}) {
        const value = await this.transport.json(`/automation/${encodePathSegment(id)}/${enabled ? "enable" : "disable"}`, {
            method: "POST",
            body: { owner },
            expectedStatus: 200,
            ...signalOptions(options.signal)
    ...
    async invoke(invocation, options = {}) {
        const value = await this.transport.json(`/automation/${encodePathSegment(invocation.automationId)}/invoke`, {
            method: "POST",
    ...
    async events(id, after = 0, options = {}) {
        ...
        const value = await this.transport.json(`/automation/${encodePathSegment(id)}/events?after=${cursor}`, { ...signalOptions(options.signal) });
    ...
    async invocations(id, options = {}) {
        const value = await this.transport.json(`/automation/${encodePathSegment(id)}/invocations`, { ...signalOptions(options.signal) });
        return requiredArray(value, "automation invocations").map(decodeAutomationInvocation);
    }
    async export(id, options = {}) {
        const value = await this.transport.json(`/automation/${encodePathSegment(id)}/export`, { ...signalOptions(options.signal) });
        return requiredArray(value, "automation export").map(decodeAutomationJournalEntry);
    }
    async exportAutomation(id, options = {}) {
        return this.export(id, options);
    }
}
export function encodeAutomationDefinition(definition) {
    return {
        id: definition.id,
...
export function encodeAutomationInvocation(invocation) {
    return {
        id: invocation.id,
...
function encodeAutomationTrigger(trigger) {
    return {
        kind: trigger.kind,
...
function encodeAutomationBudget(budget) {
    return {
...
function encodeAutomationSecretScope(scope) {
    return {
...
export function decodeAutomationDefinition(value) {
    ...
    return {
        ...
        cancellationPolicy: requiredString(record.cancellationPolicy, "automation.cancellationPolicy"),
...
export function decodeAutomationInvocation(value) {
    ...
    return {
        ...
        source: requiredString(record.source, "automation invocation.source"),
...
export function decodeAutomationEvent(value) {
...
export function decodeAutomationAuditEvent(value) {
    return decodeAutomationEvent(value);
}
export function decodeAutomationJournalEntry(value) {
...
function decodeAutomationTrigger(value) {
    ...
    return {
        ...record,
        kind: requiredString(record.kind, "automation trigger.kind"),
...
function decodeAutomationBudget(value) {
...
function decodeAutomationSecretScope(value) {
