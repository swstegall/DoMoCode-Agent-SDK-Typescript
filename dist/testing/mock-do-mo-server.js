...
export class MockDoMoServer {
    token;
    baseURL = "http://mock.domocode.test";
    protocolVersion;
    version;
    capabilities;
    corsOrigins;
    ...
    emit(sessionId, event) {
        ...
        if (session.events.length > 2048)
            session.events.shift();
        if (event.type === "permission_request" && "id" in event)
            session.permissions.set(event.id, event);
        if (event.type === "permission_resolved" && "id" in event)
            session.permissions.delete(event.id);
    ...
    async requestPermission(sessionId, request) {
        this.emit(sessionId, { type: "permission_request", ...request });
    }
    async requestQuestion(sessionId, request) {
    ...
    async handleAuthorizedFetch(input, init) {
        if (this.closed)
            return errorResponse(503, "MockDoMoServer is closed");
        const url = new URL(input instanceof Request ? input.url : input.toString(), this.baseURL);
        if (readHeader(init?.headers, "authorization") !== `Bearer ${this.token}`)
            return new Response(null, { status: 401 });
        const method = init?.method?.toUpperCase() ?? "GET";
        const parts = url.pathname.split("/").filter(Boolean).map((part) => decodeURIComponent(part));
        try {
            if (method === "GET" && url.pathname === "/capabilities")
                return jsonResponse({ name: "domocode", version: this.version, protocolVersion: this.protocolVersion, capabilities: this.capabilities });
            ...
            if (method === "GET" && tail[0] === "events")
                return this.openEvents(session, Number(url.searchParams.get("after") ?? "0"), init?.signal ?? undefined);
            if (method === "GET" && tail[0] === "status")
                return jsonResponse(this.status(session));
            if (method === "GET" && tail[0] === "messages")
                return jsonResponse(session.messages);
            if (method === "GET" && tail[0] === "context")
                return jsonResponse({ messages: session.messages, accounting: null });
            if (method === "GET" && tail[0] === "permissions")
                return jsonResponse([...session.permissions.values()]);
            if (method === "GET" && tail[0] === "questions")
                return jsonResponse([...session.questions.values()]);
    ...
    handleGlobal(method, parts, url, init) {
        if (method === "GET" && parts[0] === "commands")
            ...
            start: (controller) => {
                ...
                for (const item of session.events)
                    if (item.sequence > after)
                        this.enqueue(state, { ...item.event, sequence: item.sequence });
                if (signal) {
    ...
    async prompt(session, body) {
        if (session.running)
            return errorResponse(409, "session is busy");
    ...
    async runPrompt(session, prompt, images) {
        ...
        for (const event of result?.events ?? [])
            this.emit(session.ref.id, event);
        if (result?.message) {
    ...
    requireSession(id) {
        ...
        if (!session)
            throw new Error(`Unknown mock session ${id}`);
        return session;
    }
}
//# sourceMappingURL=mock-do-mo-server.js.map