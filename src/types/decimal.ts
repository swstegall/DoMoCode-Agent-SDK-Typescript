...
export class ExactDecimal {
  ...
  toString(): string {
    ...
    if (split <= 0) return `${sign}0.${"0".repeat(-split)}${digits}`;
...
export function parseDecimal(value: string): ExactDecimal {
  if (!DECIMAL.test(value)) throw new SyntaxError(`Invalid decimal string: ${value}`);
  ...
  if (scale < 0) return new ExactDecimal(sign * BigInt(digits) * 10n ** BigInt(-scale), 0);
